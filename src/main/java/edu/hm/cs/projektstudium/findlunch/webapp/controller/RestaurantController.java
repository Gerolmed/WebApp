...
/**
 * The class is responsible for handling http calls related to the process of
 * adding a restaurant.
 */
@Controller
public class RestaurantController {
	...
	/**
	 * The custom user details service for restaurant users. Used to refresh the
	 * SecurityContextHolder after a restaurant is added
	 */
	@Autowired
	private RestaurantUserDetailsService customUserDetailsService;

	/**
	 * The custom restaurant validator. Handles enhanced checks not handled by the
	 ...
	 *            the http session
	 * @param model
	 *            Model in which necessary objects are placed to be displayed on the
	 *            website.
	 * @param principal
	 ...
	 *            the HttpServletRequest
	 * @param restaurant
	 *            Restaurant object to be saved. Populated by the content of the
	 *            html form field.
	 * @param bindingResult
	 *            Binding result in which errors for the fields are stored.
	 *            Populated by hibernate validation annotation and custom validator
	 *            classes.
	 * @param session
	 *            the http session
	 * @param model
	...
	@RequestMapping(method = RequestMethod.POST, path = { "/restaurant/add" }, params = { "saveRestaurant" })
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(method = RequestMethod.POST, path = { "/restaurant/edit" }, params = { "saveRestaurant" })
	public String saveRestaurant(@Valid final Restaurant restaurant, BindingResult bindingResult, final Model model,
		...
		// Wenn das Restaurant bereits vorhanden ist, wird die Blockierung nicht
		// geändert. Wenn es neu ist, ist es blockiert.
		...
		UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(updatedUserDetails, null,
	...
	/**
	 * Handles the upload of a new logo. Resolves the image format, generates the
	 * base64 string for the website. Stores the newly added image to the session.
	 *
	 * @param request
	 *            the HttpServletRequest
	 * @param restaurant
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(path = { "/restaurant/add", "/restaurant/edit" }, method = RequestMethod.POST, params = {
			"addLogo" })
	public String addLogo(final Restaurant restaurant, Model model, @RequestParam("img") MultipartFile file,
		...
		session.setAttribute("blockedFileUpload",
				fileUploadRestrictorHelper.isBlocked(request.getRemoteAddr(), session.getId()));
		...
		if (!file.getContentType().startsWith("image") || imageFormat.equals("")) {
			...
			LOGGER.error(LogUtils.getErrorMessage(request, Thread.currentThread().getStackTrace()[1].getMethodName(),
					"The logo type was invalid. Only images are allowed, but type was: " + file.getContentType()
							+ " with image format: " + imageFormat));
			return "restaurant";
		}
	...
	/**
	 * Handles a time schedule from a restaurant. Removes TimeSchedules with no
	 * OfferTimes; Sets the restaurant for each valid TimeSchedule; Sets the
	 * reference (TimeSchedule) to each valid OpeningTime
	 *
	 * @param restaurant
	...
	private void handleTimeSchedule(Restaurant restaurant, int i) {
		...
		if (t.getOfferStartTime() == null && t.getOfferEndTime() == null
				&& (t.getOpeningTimes() == null || t.getOpeningTimes().size() == 0)) {
			// remove entry
	 ...
	 * 
	 * @param model
	 *            Model in which necessary object are placed to be displayed on the
	 *            website.
	 * @param principal
	 ...
	 */
	// See Stackoverflow:
	// http://stackoverflow.com/questions/12659572/how-to-generate-a-random-9-digit-number-in-java
	...
	/**
	 * In order to prevent information disclosure this method was added. Without
	 * this method a full stack trace was shown to the user when a file bigger than
	 * the defined multipart.maxFileSize was sent by the user. This stack trace
	 * could reveal some sensitive information to a potential attacker. This is a
	 * quite unclean approach but shows the problematic.
	 *
	...
	public ModelAndView resolveException(final HttpServletRequest httpServletRequest,
			final HttpServletResponse httpServletResponse, final Object o, final Exception ex) {
		 ...
		 * sendMessageToTelegram("Exception in OfferDetailController." +
		 * " The IP-address was: " + httpServletRequest.getRemoteAddr() +
		 * " The session-ID was: " + httpServletRequest.getSession().getId()); return
		 * new ModelAndView("error");
		 */
		ex.printStackTrace();
		// If it is not a MultipartException, the exception should be handled by
		// something else and not this method.
		return null;
	}

	/**
	 * Deletes an restaurant logo from the session. It is deleted from the database
	...
	private void addDefaultLogo(Restaurant restaurant) {
		try {
			InputStream in = getClass().getClassLoader().getResourceAsStream("static/images/restaurantDefault.png");
